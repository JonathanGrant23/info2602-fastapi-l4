...
def add_category_to_todo(todo_id:int, cat_id:int, db:SessionDep, user:AuthDep):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="An error occurred while adding a category to the todo item",
        )
...
def remove_category_from_todo(todo_id:int, cat_id:int, db:SessionDep, user:AuthDep):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="An error occurred while removing a category from the todo item",
        )
    
@todo_router.get('/categories', response_model=list[CategoryResponse])
def get_categories(db:SessionDep, user:AuthDep):
    return db.exec(select(Category).where(Category.user_id==user.id)).all() 